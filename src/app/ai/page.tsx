    ...
    try {
      ...
      if (data && data.answer) {
        const item: ConversationItem = {
          query: queryText,
          answer: data.answer,
          provider: data.provider || 'TinyFish AI Agent'
        };
        setConversations((prev) => [item, ...prev]);
        setSelectedAnswer(data.answer);
        setCurrentProvider(data.provider || 'TinyFish AI Agent');
      } else {
            ...
            {conversations.map((c, i) => (
              ...
              >
                ...
                <div className="flex items-center justify-between text-[10px] font-mono text-amber-400 mt-1.5">
                  ...
                  <span className="text-neutral-400 truncate max-w-[120px]">{c.provider || 'TinyFish AI'}</span>
                </div>
              </div>
            ))}
          </div>
            ...
            {!selectedAnswer && !isLoading && (
              <div className="h-full flex flex-col items-center justify-center text-center p-6 text-xs text-neutral-400 space-y-4">
                ...
                <p className="max-w-md text-sm text-neutral-300 leading-relaxed">
                  Ask any question about Reliance business, macroeconomic trends, microeconomic spreads, cracker kinetics, or site-wide data. Answers are generated by the TinyFish AI Agent grounded in real-time web intelligence, live market telemetry, and cracker asset records.
                </p>
